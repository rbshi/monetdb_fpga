...
str UDFdoubleit_(int *dst, const int *src)
{
    ...
    std::vector<int, aligned_allocator<int> > source_hw_double_results[NUM_KERNEL];

    source_in1[0] = *src;

    for (int i = 0; i < NUM_KERNEL; i++) {
        source_hw_double_results[i].resize(dataSize);
    }

    // Initializing output vectors to zero
    for (size_t i = 0; i < NUM_KERNEL; i++) {
        std::fill(source_hw_double_results[i].begin(), source_hw_double_results[i].end(), 0);
    }
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            for (int i = 0; i < NUM_KERNEL; i++) {
                ...
                std::string krnl_name_full = krnl_name + ":{" + "krnl_fpga_op_" + cu_id + "}";
    ...
    std::vector<cl_mem_ext_ptr_t> outDoubleBufExt(NUM_KERNEL);


    std::vector<cl::Buffer> buffer_input1(NUM_KERNEL);
    std::vector<cl::Buffer> buffer_output_double(NUM_KERNEL);
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        ...
        outDoubleBufExt[i].obj = source_hw_double_results[i].data();
        outDoubleBufExt[i].param = 0;
        outDoubleBufExt[i].flags = pc[(i * 4) + 1];
    }
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        ...
        OCL_CHECK(err, buffer_output_double[i] =
                               cl::Buffer(context, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
                                          sizeof(uint32_t) * dataSize, &outDoubleBufExt[i], &err));
    }
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        ...
        OCL_CHECK(err, err = krnls[i].setArg(1, buffer_output_double[i]));
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_output_double[i]}, CL_MIGRATE_MEM_OBJECT_HOST));
    }
    q.finish();
    ...
    // write back to udf
    *dst = source_hw_double_results[0][0];
